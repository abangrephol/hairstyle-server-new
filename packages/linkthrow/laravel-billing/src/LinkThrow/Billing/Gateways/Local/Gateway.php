<?php namespace LinkThrow\Billing\Gateways\Local;

use LinkThrow\Billing\Gateways\GatewayInterface;
...
class Gateway implements GatewayInterface
{
    ...
    {
        if (null === $connection) {
            $connection = Config::get('billing.gateways.local');
        }
    ...
    {
        $delay = Config::get('billing.gateways.local.api_delay_ms');
        if (empty($delay)) {
            return;
        }
