...
namespace App\Repositories\Backend\Reseller\Reseller;
 ...
 */
class EloquentResellerRepository implements ResellerContract
{
    /**
    ...
    {
        $users = Reseller::search('Reseller',['roles.name'])
            ->where('status', $status)
    ...
    public function getAllResellers ($order_by = 'id', $sort = 'asc')
    {
        return Reseller::search('Reseller',['roles.name'])
            ->where('deleted_at',null)
            ->orderBy($order_by, $sort)
            ->get();
    }
    ...
    public function create($input)
    {
        ...
        if ($user->save()) {
            ...
            $this->validateRoleAmount($user, [3]);
